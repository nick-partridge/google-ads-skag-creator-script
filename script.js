function main() {
    ...
    //CREATE AD GROUPS THAT CONTAIN ALL 3 MATCH TYPES WITHIN IT - 'true' or 'false' (WITHOUT QUOTES) ARE THE OPTIONS. IF SET TO FALSE, ONLY EXACT MATCH WILL BE USED
...

      {
        ...
        function text_ad_builder(new_ad_group) {
          ...
          if (pre_skag_adgroup_iterator.hasNext()) {
            ...
            var adIterator = pre_skag_adgroup.ads().withCondition('Type=EXPANDED_TEXT_AD').withCondition("Status = ENABLED").get();
            while (adIterator.hasNext()) {
              ...
              var keyword_headline = add_query_to_headline === true ? filtered_query.Query.toProperCase() : getExpandedTextAd.headline_one;

              try {
                return new_ad_group.newAd().expandedTextAdBuilder()
                  .withHeadlinePart1(filtered_query.Query.length <= 30 ? keyword_headline : getExpandedTextAd.headline_one)
                ...
                Logger.log("No '" + filtered_query.CampaignName + em_only_campaign_suffix + "' campaign for the '" + filtered_query.Query + "' query - either create a campaign with that name, or change the 'em_only_campaign' setting to false");
              }
            }
          }
        }
